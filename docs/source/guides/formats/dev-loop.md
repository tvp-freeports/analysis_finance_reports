...
Four commands, in this order, over and over. {doc}`../../reference/cli/index` is the option-by-option reference; this
...
diff ({doc}`../../reference/design/blocks`).
...
also what the grants of {doc}`../../overview/trust` are *about*: a grant is a claim about the content of
exactly those bytes.
```
