...
({doc}`../../reference/design/segments`). There are three **levels** at which a segment can be specified, and they
differ only in how much of the algorithm you supply yourself:

| Level | The algorithm | Its parameters | You write | Page |
|---|---|---|---|---|
| **structured** | in the library, fixed | columns of a CSV | rows in a spreadsheet | {doc}`advanced/structured` |
| **semistructured** | in the library *by name*, or your own | YAML | a name and a configuration | {doc}`advanced/semistructured` |
| **unstructured** | in the repository | in the code | Python | {doc}`advanced/unstructured` |
...
({doc}`../../reference/design/classification`).
...
the next one ({doc}`../../reference/design/schedule`).
...
and on what basis — {doc}`../../overview/trust` for the mechanism, {doc}`../../reference/cli/index` for the commands.
...
database problem rather than an extraction one. See {doc}`../input-db/index`.
...
everything the page-as-unit assumption buys ({doc}`../../reference/design/promises`).
