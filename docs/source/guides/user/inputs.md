...
design, not an omission: see {doc}`../../reference/design/formats-as-plugins`.
...
| **input database** | which companies to look for, and how to recognise each | `--db-directory` / `-I`, plus `--target-list` / `-T` | {doc}`../input-db/index` |
...
{doc}`../input-db/index` is the full description.
