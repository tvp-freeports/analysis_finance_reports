...
from the document specifier ({doc}`../../guides/user/documents`):
...
Several documents in one invocation is not the same as {doc}`batch mode <../../guides/user/advanced/batch>`, and the
