...
Why the engine has the shape it has. Not *how to run it* — that is {doc}`../../guides/user/index` — and not
which libraries it is built from, which is a question for contributors and lives in
{doc}`the implementation notes <../../guides/engine/advanced/implementation-notes>`.
