...
Assuming {doc}`install` is done and the two {doc}`../guides/user/inputs` exist.
...
: the document. A bare path here means *this local file*, made absolute, and its own text becomes
  ...
  documents. The full grammar is in {doc}`../guides/user/documents`.
...
rows, so *"nothing found"* and *"no such table"* stay distinguishable. {doc}`../guides/user/output` is the full
list and the rules.

`.log.csv` is not an output table: it is the extraction's own record of what it skipped and where.
It lands **next to the output** because it is a product of the run. {doc}`../guides/user/logging` explains how to
read it.
...
resolution and fund deduplication. See {doc}`../reference/design/multidocument`.
...
| holdings attributed to the wrong company | the input database's order, not the extraction — see {doc}`../guides/user/inputs` |

Raise the verbosity before guessing: `-v` narrates the run, `-vv` and `-vvv` more. {doc}`../guides/user/logging`.
