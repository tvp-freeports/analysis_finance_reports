...
| GnuPG and `jq` | `freeports-validate` only | its Python dependencies come with it; these two must come from your system — see {doc}`../reference/cli/index` |
...
`make help` lists the rest, and {doc}`../guides/engine/build` explains the arrangement. If anything looks
...
with the engine. See {doc}`../guides/user/inputs`.
