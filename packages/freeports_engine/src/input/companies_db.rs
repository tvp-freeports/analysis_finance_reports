...
fn read_csv(path: &Path) -> PyResult<DataFrame> {
...
fn required_str_column(df: &DataFrame, name: &str, path: &Path) -> PyResult<Vec<String>> {
    let series = df
        .column(name)
        .map_err(|_| PyValueError::new_err(format!("{}: missing required column '{name}'", path.display())))?
        .str()
        .map_err(|e| PyValueError::new_err(format!("{}: column '{name}' is not text: {e}", path.display())))?;
    series
        .into_iter()
        .enumerate()
        .map(|(i, v)| v.map(str::to_string).ok_or_else(|| PyValueError::new_err(format!("{}: row {} has an empty '{name}'", path.display(), i + 1))))
        .collect()
}

fn optional_str_column(df: &DataFrame, name: &str, len: usize, path: &Path) -> PyResult<Vec<Option<String>>> {
    match df.column(name) {
        Err(_) => Ok(vec![None; len]),
        Ok(col) => {
            let series = col.str().map_err(|e| PyValueError::new_err(format!("{}: column '{name}' is not text: {e}", path.display())))?;
            Ok(series.into_iter().map(|v| v.map(str::to_string)).collect())
        }
    }
}

fn require_already_normalized(field: &str, value: &str, context: &str) -> PyResult<()> {
    if normalization::deep_normalize_string(value) != value {
        return Err(PyValueError::new_err(format!("{context}: {field} '{value}' is not already normalized")));
    }
    Ok(())
}

fn require_bud_contained_in_name(bud: &str, name: &str) -> PyResult<()> {
    let n_name = normalization::deep_normalize_string(name);
    if !n_name.contains(bud) {
        return Err(PyValueError::new_err(format!("Bud '{bud}' is not contained in company name '{name}' (normalized: '{n_name}')")));
    }
    Ok(())
}
...
/// `\bairbnb\b`).
fn require_regex_matches_name(pattern: &str, name: &str) -> PyResult<()> {
    let n_name = normalization::deep_normalize_string(name);
    let re = onig::Regex::new(pattern).map_err(|e| PyValueError::new_err(format!("invalid regex '{pattern}' for company '{name}': {e}")))?;
    if re.find(&n_name).is_none() {
        return Err(PyValueError::new_err(format!("regex '{pattern}' does not match company name '{name}' (normalized: '{n_name}')")));
    }
    Ok(())
}

fn require_valid_date(value: &str, context: &str) -> PyResult<()> {
    let parts: Vec<&str> = value.split('-').collect();
    let malformed = || PyValueError::new_err(format!("{context}: '{value}' is not a valid YYYY-MM-DD date"));
...
fn load_companies(input_db_directory: &Path) -> PyResult<Vec<CompanyRow>> {
    ...
    for i in 0..names.len() {
        ...
        if !seen.insert(name.clone()) {
            return Err(PyValueError::new_err(format!("{}: duplicate company name '{name}'", path.display())));
        }
...
fn load_additional(input_db_directory: &Path, subdir: &str, file_name: &str, value_column: &str, company_names: &HashSet<String>) -> PyResult<Vec<(String, String)>> {
    ...
    companies
        ...
        .map(|(company_name, value)| {
            if !company_names.contains(&company_name) {
                return Err(PyValueError::new_err(format!("{}: '{company_name}' is not a known company", path.display())));
            }
...
fn load_lists(input_db_directory: &Path) -> PyResult<HashSet<String>> {
    ...
    for (i, name) in names.iter().enumerate() {
        if !seen.insert(name.clone()) {
            return Err(PyValueError::new_err(format!("{}: duplicate list name '{name}'", path.display())));
        }
...
fn load_company_to_list(input_db_directory: &Path, list_names: &HashSet<String>, company_names: &HashSet<String>) -> PyResult<Vec<(String, String)>> {
    ...
    lists
        ...
        .map(|(list_name, company_name)| {
            if !list_names.contains(&list_name) {
                return Err(PyValueError::new_err(format!("{}: '{list_name}' is not a known list", path.display())));
            }
            if !company_names.contains(&company_name) {
                return Err(PyValueError::new_err(format!("{}: '{company_name}' is not a known company", path.display())));
            }
...
fn load_markets(input_db_directory: &Path) -> PyResult<HashSet<String>> {
    ...
    for name in names {
        if !seen.insert(name.clone()) {
            return Err(PyValueError::new_err(format!("{}: duplicate market name '{name}'", path.display())));
        }
    }
    Ok(seen)
}

fn load_tickers(input_db_directory: &Path, market_names: &HashSet<String>, company_names: &HashSet<String>) -> PyResult<Vec<(String, String, String)>> {
    ...
    (0..markets.len())
        .map(|i| {
            ...
            if !market_names.contains(&market_name) {
                return Err(PyValueError::new_err(format!("{}: '{market_name}' is not a known market", path.display())));
            }
            if !company_names.contains(&company_name) {
                return Err(PyValueError::new_err(format!("{}: '{company_name}' is not a known company", path.display())));
            }
            if symbol_re.find(&symbol).is_none() {
                return Err(PyValueError::new_err(format!("{}: '{symbol}' is not a valid ticker symbol (expected 2-6 uppercase letters)", path.display())));
            }
...
fn load_target_companies(input_db_directory: &Path, target_lists: &[String]) -> PyResult<Vec<TargetCompanyInput>> {
...
pub fn py_get_target_companies(py: Python<'_>, input_db_directory: PathBuf, target_lists: &Bound<'_, PyAny>) -> PyResult<Py<PyList>> {
    ...
    let result = crate::formats_utils::text_filter::matcher::CompanyMatchInfos::compile_from_rows(rows)?;
    let items: Vec<Py<crate::formats_utils::text_filter::matcher::CompanyMatchInfos>> =
        result.into_iter().map(|c| Py::new(py, c)).collect::<PyResult<_>>()?;
