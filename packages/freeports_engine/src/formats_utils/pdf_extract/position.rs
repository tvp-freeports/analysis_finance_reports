...
fn validate_positive(name: &str, v: Option<f64>) -> PyResult<()> {
    if let Some(v) = v
        && v <= 0.0 {
...
impl InputArea {
    ...
    fn new(x_min: Option<f64>, x_max: Option<f64>, y_min: Option<f64>, y_max: Option<f64>) -> PyResult<Self> {
        validate_positive("x_min", x_min)?;
        validate_positive("x_max", x_max)?;
        validate_positive("y_min", y_min)?;
        validate_positive("y_max", y_max)?;
