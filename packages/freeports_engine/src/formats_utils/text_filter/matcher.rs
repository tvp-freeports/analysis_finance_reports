...
fn compile_regex_pattern(p: &str) -> Result<Regex, PyErr> {
    ...
    Ok(Regex {
        reference: Arc::new(
            ...
            .map_err(|e| PyErr::new::<PyException, _>((p.to_string(), e.description().to_string())))?,
        ),
...
fn compile_symbol_pattern(p: &str) -> Result<Regex, PyErr> {
...
impl CompanyMatchInfos {
    ...
    pub fn compile_from_target_companies(companies: Vec<TargetCompanyInput>) -> Result<Vec<Self>, PyErr> {
        companies
            .into_iter()
            .map(|company| {
...
impl CompanyMatchInfos {
    ...
    pub fn compile_from_rows(rows: Vec<CompanyRowForCompilation>) -> Result<Vec<Self>, PyErr> {
        let companies = rows
            ...
            .collect();
        Self::compile_from_target_companies(companies)
    }
...
/// reach this via `py.import("freeports_engine")...call_method1("match_company", ...)`, re-import
