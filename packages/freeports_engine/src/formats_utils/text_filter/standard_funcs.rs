...
/// **Bug found and fixed at the root — a repeat of a lesson `deserialize/cast.rs` already
/// documents (user confirmed, 2026-08-19)**: the field-extraction warnings/errors below were
/// initially replaced with `tracing::warn!`/no logging at all, on the same wrong assumption
/// `cast.rs`'s own header warns against: that these calls are cosmetic. They aren't — every
/// `.log.csv` fixture in the formats repo is built from *exactly* these Python
/// `logging`-module calls (via a `logging.FileHandler` at `WARNING` level, wired up in
/// `_internals/cli/main.py`, reading `LOG_ADAPT_INVESTMENT_INFOS`'s `row`/`company`/
/// `company_match` plus each call's own `extra={...}`) — confirmed the hard way: 4 formats'
/// `test_pipeline` failed on `.log.csv` row-count mismatches (missing exactly the rows these
/// calls were supposed to produce) with every other output file (`investments.csv` included)
/// matching byte-for-byte. Fixed by routing these specific calls through real Python `logging`
/// and mutating the real `LOG_ADAPT_INVESTMENT_INFOS` singleton, exactly like the original.
/// `logger.debug(...)` calls elsewhere in the original are *not* replicated: the CSV handler is
/// `WARNING`-level, so debug calls never reach it — verified by reading the handler setup, not
/// assumed this time.
fn text_filter_logger(py: Python<'_>) -> PyResult<Bound<'_, PyAny>> {
    py.import("logging")?.call_method1("getLogger", ("freeports._internals.formats.utils.text_filter.standard_funcs",))
}
...
fn log_adapt_investment_infos(py: Python<'_>) -> PyResult<Bound<'_, PyAny>> {
    py.import("freeports._internals.core.logging")?.getattr("LOG_ADAPT_INVESTMENT_INFOS")
}
...
/// `freeports_engine.core.MatchFund` it wraps) — deliberately, even though this crate could
...
#[pyclass(name = "filter_block_type", module = "freeports_engine")]
...
#[pyclass(name = "filter_block_types", module = "freeports_engine")]
...
#[pyclass(name = "filter_block_type_call", module = "freeports_engine")]
...
#[pyclass(name = "filter_block_types_call", module = "freeports_engine")]
...
#[pyclass(name = "fund_filter_data", module = "freeports_engine")]
...
#[pyclass(name = "fund_filter_data_call", module = "freeports_engine")]
...
#[pyclass(name = "investment_fund_filter_data", module = "freeports_engine")]
...
#[pyclass(name = "investment_fund_filter_data_call", module = "freeports_engine")]
...
impl TextFilterInvestmentsStandard {
    ...
    fn run_loop<'py>(&self, py: Python<'py>, pdf_blocks: &[Py<PyAny>], targets: &Bound<'py, PyAny>) -> PyResult<Vec<Bound<'py, TextBlock>>> {
        ...
        // the (still-working, but wasteful) `py.import("freeports_engine")` round-trip with a
        ...
        let log_adapt = log_adapt_investment_infos(py)?;
        ...
        while i < table.len() as i64 - 1 {
            ...
            if let Some(company) = company {
                log_adapt.setattr("company", &company)?;
                log_adapt.setattr("company_match", &content)?;
                if split {
                ...
                match self.extract_field(py, &table, i, (row, col), n_cols) {
                    ...
                    Err(err) if err.is_instance_of::<ExpectedTextBlockNotFound>(py) => {
                        text_filter_logger(py)?.call_method1("warning", (translate(py, "Skipping line...")?,))?;
                    }
                    Err(err) => return Err(err),
                }
                log_adapt.setattr("company_match", py.None())?;
                log_adapt.setattr("company", py.None())?;
            }
            i += 1;
    ...
    fn extract_field<'py>(&self, py: Python<'py>, table: &PdfBlocksTable, i: i64, base: (i64, i64), n_cols: i64) -> PyResult<Bound<'py, TextBlock>> {
        ...
        let manco = match &anchor {
            ...
            _ => {
                text_filter_logger(py)?.call_method1(
                    "error",
        ...
        let market_value = match market_value {
            ...
            None => {
                ...
                extra.set_item("field", "Market value")?;
                let kwargs = PyDict::new(py);
                kwargs.set_item("extra", &extra)?;
                text_filter_logger(py)?.call_method("error", ("Field not found",), Some(&kwargs))?;
        ...
        for (pos, name) in [
            ...
            if let Some(pos) = pos {
                let (resolved, value) = resolve(pos);
                if value.is_none() {
                    let extra = PyDict::new(py);
                    match resolved {
                        Some((row, col)) => {
                            extra.set_item("row", row)?;
                            extra.set_item("col", col)?;
                        }
                        None => {
                            extra.set_item("row", py.None())?;
                            extra.set_item("col", py.None())?;
                        }
                    }
                    extra.set_item("field", name)?;
                    let kwargs = PyDict::new(py);
                    kwargs.set_item("extra", &extra)?;
                    text_filter_logger(py)?.call_method(
                        "error",
                        (translate(py, "Expected field not found, replacing with None...")?,),
                        Some(&kwargs),
                    )?;
                }
...
mod tests {
    ...
    fn sfdr_article_strips_prefix_and_requires_investment_fund_match() {
        Python::attach(|py| {
            ...
            let equity = py.get_type::<crate::output::classes::investment::Equity>().call((), Some(&kwargs)).unwrap().unbind();
    ...
    // `freeports_engine.core.TextBlock`), and `cargo test`'s embedded interpreter has its *own*
    ...
    fn assets_standard_filters_by_fund_and_extracts_currency() {
        Python::attach(|py| {
            ...
            let fund = crate::output::classes::fund::Fund::new(&name).unwrap();
    ...
    /// Regression test for the `.log.csv` fixture regression (user confirmed, 2026-08-19): when
    /// a matched company's field extraction fails, the skip path must go through real Python
    /// `logging` (WARNING level, "Skipping line...") — not `tracing` — because that's what
    /// `.log.csv`'s fixtures are built from. Verified by attaching a real `logging.Handler` and
    /// checking the emitted record directly, not just that nothing crashes.
    #[test]
    fn investments_standard_logs_skipping_line_via_python_logging_on_extraction_failure() {
        Python::attach(|py| {
            crate::test_support::ensure_freeports_imported(py);
            // market_value_pos points one column past the only real column, so field extraction
            // always fails for a matched row.
            let row0 = table_row(py, 0, 0, "Italy Btps bond", true).unbind();
            let blks = PyList::new(py, [row0]).unwrap();

            let records = py.eval(std::ffi::CString::new("[]").unwrap().as_c_str(), None, None).unwrap();
            let logging = py.import("logging").unwrap();
            let handler_cls = py
                .eval(
                    std::ffi::CString::new(
                        "type('CollectingHandler', (__import__('logging').Handler,), {'emit': lambda self, record: self.records.append(record)})",
                    )
                    .unwrap()
                    .as_c_str(),
                    None,
                ...
                .unwrap();
            let handler = handler_cls.call0().unwrap();
            handler.setattr("records", &records).unwrap();
            handler.call_method1("setLevel", (logging.getattr("WARNING").unwrap(),)).unwrap();
            let logger = text_filter_logger(py).unwrap();
            logger.call_method1("addHandler", (&handler,)).unwrap();

            let inv = TextFilterInvestmentsStandard::new(5, None, None, None, None, true, false).unwrap();
            let targets = company_match_infos(py, &[("Italy btps", "italy btps")]);
            let result = inv.run_loop(py, &[blks.get_item(0).unwrap().unbind()], targets.as_any());

            logger.call_method1("removeHandler", (&handler,)).unwrap();

            let result = result.unwrap();
            assert!(result.is_empty(), "extraction should fail and skip the line");

            let records: Vec<Bound<'_, PyAny>> = records.try_iter().unwrap().map(|r| r.unwrap()).collect();
            assert_eq!(records.len(), 2, "expected the 'Field not found' error + 'Skipping line...' warning");
            let messages: Vec<String> = records.iter().map(|r| r.call_method0("getMessage").unwrap().extract().unwrap()).collect();
            assert!(messages.contains(&"Field not found".to_string()));
            assert!(messages.contains(&"Skipping line...".to_string()));
        });
    }
}
