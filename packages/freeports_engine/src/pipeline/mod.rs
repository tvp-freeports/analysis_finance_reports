        ...
        for step in 0..n_steps {
            ...
            for (pt, page_triples) in step_dict.iter() {
                ...
                for triple in page_triples.try_iter()? {
                    ...
                    py.import("freeports._internals.core.logging")?.getattr("LOG_CONTEXTUAL_INFOS")?.setattr("page", &page_n)?;
                    ...
                    py.import("freeports._internals.core.logging")?.getattr("LOG_CONTEXTUAL_INFOS")?.setattr("page", py.None())?;
                }
            }
    ...
    fn run_documents(&self, py: Python<'_>, docs: &Bound<'_, PyAny>, target_companies: &Bound<'_, PyAny>, format_name: &str) -> PyResult<Py<PyList>> {
        ...
        for (doc_name, page_n, doc_name_obj, page_n_obj) in &entries {
            ...
            for r in page_result_items.try_iter()? {
                ...
                } else if r.is_instance_of::<crate::output::classes::investment::Equity>()
                    || r.is_instance_of::<crate::output::classes::investment::Bond>()
                {
                    last_page.getattr("investments")?.call_method1("append", (&r,))?;
                } else if r.is_instance_of::<crate::output::classes::assets_manager::ManagementCompany>()
                    || r.is_instance_of::<crate::output::classes::assets_manager::InvestmentsManager>()
                {
                    last_page.getattr("assets_managers")?.call_method1("append", (&r,))?;
                } else if r.is_instance_of::<crate::output::classes::fund::Fund>() {
                    last_page.getattr("funds")?.call_method1("append", (&r,))?;
                } else if r.is_instance_of::<crate::output::classes::fund_sfdr_classification::FundSfdrClassification>() {
                    last_page.getattr("funds_sfdr_classification")?.call_method1("append", (&r,))?;
                } else if r.is_instance_of::<crate::output::classes::fund_esg_indicator::FundEsgIndicator>() {
                    last_page.getattr("funds_esg_indicators")?.call_method1("append", (&r,))?;
                } else if r.is_instance_of::<crate::output::classes::fund_assets::FundAssets>() {
                    last_page.getattr("funds_assets")?.call_method1("append", (&r,))?;
                } else if r.is_instance_of::<crate::output::classes::fund_change_name::FundRename>()
                    || r.is_instance_of::<crate::output::classes::fund_change_name::FundMerge>()
                {
...
mod tests {
    ...
    /// `"freeports_engine.Fund(...)"` would go through `sys.modules['freeports_engine']` — the
    ...
    fn fund<'py>(py: Python<'py>, name: &str) -> Bound<'py, PyAny> {
        py.get_type::<crate::output::classes::fund::Fund>().call1((name,)).unwrap()
    }
    ...
    fn run_documents_dispatches_management_company_as_assets_manager() {
        Python::attach(|py| {
            ...
            let mc = py
                .get_type::<crate::output::classes::assets_manager::ManagementCompany>()
    ...
    fn run_documents_dispatches_fund_rename_as_fund_change_name() {
        Python::attach(|py| {
            ...
            let rename = py
                .get_type::<crate::output::classes::fund_change_name::FundRename>()
                .call1(("Old", "New", date))
                .unwrap();
    ...
    fn run_documents_dispatches_equity_as_investment() {
        Python::attach(|py| {
            ...
            let equity = py.get_type::<crate::output::classes::investment::Equity>().call((), Some(&kwargs)).unwrap();
    ...
    fn run_documents_dispatches_fund_sfdr_classification() {
        Python::attach(|py| {
            ...
            let sfdr = py
                .get_type::<crate::output::classes::fund_sfdr_classification::FundSfdrClassification>()
                .call1(("F", article))
                .unwrap();
    ...
    fn run_documents_dispatches_fund_esg_indicator() {
        Python::attach(|py| {
            crate::test_support::ensure_freeports_imported(py);
            let esg = py
                .get_type::<crate::output::classes::fund_esg_indicator::FundEsgIndicator>()
    ...
    fn run_documents_dispatches_fund_assets() {
        Python::attach(|py| {
            ...
            kwargs.set_item("currency", currency).unwrap();
            let fund_assets = py.get_type::<crate::output::classes::fund_assets::FundAssets>().call((), Some(&kwargs)).unwrap();
    ...
    fn run_documents_resolves_promises_across_the_whole_document() {
        Python::attach(|py| {
            ...
            let fund_with_promise = py.get_type::<crate::output::classes::fund::Fund>().call1((promise,)).unwrap();
    ...
    // constructs `Pipeline` objects via the *actually installed* `freeports_engine.cpython-*.so`
    // (whatever `sys.path` resolves `import freeports_engine` to), a **different compiled
    ...
    // is `false` even though both print as `<class 'freeports_engine.Pipeline'>`, and
    ...
    // `run_job_attached`) -- through `py.import("freeports_engine").getattr("core")
    ...
    fn algorithm_load_end_to_end_against_a_complete_small_formats_repo() {
        Python::attach(|py| {
            ...
            let core = py.import("freeports_engine").unwrap().getattr("core").unwrap();
            let alg = core
                .getattr("Algorithm")
                .unwrap()
