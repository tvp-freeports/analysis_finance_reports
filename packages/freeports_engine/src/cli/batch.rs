//! Reads `--batch`'s CSV file and turns each row into a fully-validated per-job
//! `FreeportsConfig`, overlaid on top of the shared base config (mirrors `cli/main.py`'s
//! `batch_job_confs`).
...
use super::job_config::{self, JobConfigError};
use super::partial_config::{ConfigLocations, ConfigSource, PartialConfig};

/// No `py: Python<'_>` involved anywhere in this module: CSV reading is plain Rust I/O, and
/// [`FreeportsConfig::build`] (the one call here that can reach Python) attaches its own token
/// where it actually needs one — see that function's doc comment. Each variant already carries a
/// fully descriptive inner error (`csv::Error`/[`JobConfigError`]/[`FreeportsConfigError`] all
/// have their own specific `Display`); this only adds which *stage* of loading the batch file
/// failed.
#[derive(Debug)]
pub enum BatchError {
    Csv(csv::Error),
...
pub fn load_batch_jobs(base: &PartialConfig, batch_file: &Path) -> Result<Vec<FreeportsConfig>, BatchError> {
    ...
    for record in reader.records() {
        ...
        let row_config = job_config::parse_row(&row)?;
