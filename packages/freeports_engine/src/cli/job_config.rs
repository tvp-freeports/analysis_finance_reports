//! Rust port of `_internals/cli/conf_parse.py`'s `FreeportsJobConfig` — one row of a `--batch`
//! CSV file, each row being (per the user's framing, 2026-08-20) equivalent to invoking the
//! command again with different inputs. See `agent-memory/rust-native-binary-plan.md`, Fase E,
//! punto 3b-vi.
//!
//! **Redesign, restoring a comprehensively broken feature (user confirmed, 2026-08-20)**: reading
//! the Python original and testing it directly showed batch mode's per-row document/prefix
//! overrides don't work *at all*:
//! - `report`/`prefix out` columns map to a `PREFIX_OUT` key that `FreeportsJobConfig` never
//!   declares as a field — pydantic's default `extra="ignore"` drops it silently.
//! - A `pdf` cell containing the original's `DOC_SPEC_SEPARATOR` (`"|"`, meant to list multiple
//!   paths in one row) is popped into a `DOCUMENT_SPECS` key that isn't a declared field either —
//!   discarded, worse than doing nothing.
//! - Even a single `url`/`pdf` value, once validated, never reaches `FreeportsConfig.INPUT_REPORTS`
//!   — the same dead end already found and fixed for file/env config (see `file_config.rs`'s doc
//!   comment).
//!
//! The fix folds the document column into the *same* `input_reports: Vec<DocumentSpec>` slot
//! `--input` and file/env config already use, splitting a cell on `;` for multiple documents that
//! must be parsed *together* as one job (the user's stated reason multi-document jobs exist:
//! pages whose information isn't independent, e.g. a report split across two PDFs) — `;` rather
//! than `DocumentSpec`'s own `|` to avoid the two separators colliding when nested.
//!
//! Two further simplifications the user asked for once documents carry their own structure:
//! - `PREFIX_OUT` is dropped entirely rather than fixed: a CSV cell can now hold a full
//!   `DocumentSpec` (with its own `name`, already used as the `Report` output column — see
//!   `conf_parse.rs`), so a separate row-level prefix would duplicate what `DocumentSpec::name`
//!   now does. The two collapse into one concept instead of keeping both.
//! - The `url`/`pdf` column split is dropped too, for the same reason: both used to exist only
//!   because neither alone could express "a URL *and* a local path", something one `DocumentSpec`
//!   string already expresses on its own (`url|path|name`). One `input` column, parsed exactly
//!   like `--input`, replaces both.

use std::collections::BTreeMap;

use super::conf_parse::{self, ConfigError, DocumentSpec};
use super::partial_config::PartialConfig;

/// Splits one CSV cell into multiple documents meant to be parsed together as a single job.
/// Distinct from [`super::conf_parse::DocumentSpec`]'s own `|` segment separator so a cell like
/// `http://a.com/x.pdf|local.pdf|Name;http://b.com/y.pdf` unambiguously means "two documents,
/// the first with an explicit path and name" rather than being swallowed by one flat split.
pub const JOB_DOCUMENT_SEPARATOR: char = ';';

/// Every invalid cell value is a [`ConfigError`] — the same enum `cli::cmd_config`,
/// `cli::env_config`, and `cli::file_config` wrap under their own `InvalidField` variant, so a bad
/// `input`/`save pdf` cell in a batch file produces exactly the same message as the equivalent
/// `--flag`, env var, or YAML key would.
...
fn parse_documents(cell: &str, column: &'static str) -> Result<Vec<DocumentSpec>, JobConfigError> {
    cell.split(JOB_DOCUMENT_SEPARATOR)
        .map(str::trim)
        .filter(|s| !s.is_empty())
        .map(|s| s.parse::<DocumentSpec>().map_err(|source: super::conf_parse::DocumentSpecError| JobConfigError::InvalidField { column, source: source.into() }))
        .collect()
}

fn parse_bool(value: &str, column: &'static str) -> Result<bool, JobConfigError> {
    conf_parse::parse_bool_alias(value).map_err(|source| JobConfigError::InvalidField { column, source })
}

/// Mirrors `FreeportsJobConfig.__init__`: `row` is one CSV row, column names matched
/// case-insensitively and trimmed (as `csv.DictReader` headers would be), values always strings.
