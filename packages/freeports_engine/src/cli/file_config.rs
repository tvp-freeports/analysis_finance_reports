//! Rust port of `_internals/cli/conf_parse.py`'s `FreeportsFileConfig` — locating and loading the
//! YAML config file. See `agent-memory/rust-native-binary-plan.md`, Fase E, punto 3b-iii.
//!
//! `find_config`'s 3-tier search order is ported as-is: current directory (a couple of accepted
//! filename patterns), then XDG config directories on POSIX / `LOCALAPPDATA`+`PROGRAMDATA` on
//! Windows, then `/etc` on POSIX / the Windows system directory. The XDG search is implemented
//! directly against `XDG_CONFIG_HOME`/`XDG_CONFIG_DIRS` (matching `pyxdg`'s
//! `BaseDirectory.load_config_paths` order) rather than pulling in a crate for it — it's a handful
//! of environment-variable reads with a well-defined fallback, not worth a dependency.
//!
//! **YAML key mapping (redesigned, see `conf_parse.rs`/`partial_config.rs` doc comments)**: the
//! Python original's `_map_names` dict maps `url` (but, confirmed by reading it closely, *not*
//! `pdf` — a config file with a `pdf:` key raises a raw `KeyError` loading it, since `_map_names`
//! is looked up unconditionally for every YAML key present) onto a field that never actually
//! reaches `FreeportsConfig` (see `partial_config.rs`'s doc comment for the full empirically
//! confirmed chain). Both are replaced here by one `input_report` key, parsed with the same
//! [`super::conf_parse::DocumentSpec`] grammar `--input` uses, that *does* reach `INPUT_REPORTS`.
//! An unrecognized key anywhere in the YAML mapping is a clean [`FileConfigError::UnknownKey`],
//! not the original's raw `KeyError` — clear error messages over a crash are not a design
//! trade-off worth reproducing.

use std::path::{Path, PathBuf};

use onig::Regex;

use super::conf_parse::{self, ConfigError, DocumentSpec, OutFlags, OutStructureMode, Verbosity};
...
fn as_positive_u32(value: &serde_yaml::Value, key: &'static str) -> Result<u32, FileConfigError> {
    ...
    conf_parse::validate_workers(n).map_err(|source| FileConfigError::InvalidField { key, source })
}
    ...
    for (raw_key, raw_value) in mapping {
        ...
        match key {
            ...
            "input_report" => {
                ...
                let spec = s
                    .parse::<DocumentSpec>()
                    .map_err(|source: super::conf_parse::DocumentSpecError| FileConfigError::InvalidField { key: "input_report", source: source.into() })?;
