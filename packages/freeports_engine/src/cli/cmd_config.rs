//! Rust port of `_internals/cli/conf_parse.py`'s `FreeportsCmdConfig` — the command-line
//! argument parser (`argparse` → `clap`, derive-based, per the Fase E design decision) and its
//! mapping onto [`PartialConfig`]. See `agent-memory/rust-native-binary-plan.md`, Fase E, punto
//! 3b-v.
//!
//! Two confirmed bugs in the Python original are fixed here, not reproduced (per the "fix
//! unambiguous typos at the root" policy the user confirmed for this same file's other bugs):
//! - **`--config` was a complete no-op.** `argparse` parsed it into `args.config`, but
//!   `FreeportsCmdConfig`'s field/mapping table never referenced it at all — the value was parsed
//!   and silently discarded, so only the `FREEPORTS_CONFIG_FILE` env var ever actually worked to
//!   pick a config file. Fixed: `--config` now sets [`PartialConfig::config_file`].
//! - **`-q` (documented "Decrease verbosity") actually increased it.** The original computes
//!   `increase_verbosity = args["q"]` — the *positive* occurrence count — instead of negating it,
//!   so `-q` raised the verbosity level exactly like `-v` would. Confirmed empirically: with the
//!   default verbosity of 2, `-q` produced `VERBOSITY = 3`, not `1`. Fixed: `-q`'s count is
...
pub fn from_args(args: CliArgs, default_verbosity: u8) -> Result<PartialConfig, CmdConfigError> {
    ...
    let n_workers = match args.workers {
        None => None,
        Some(n) => Some(
            conf_parse::validate_workers(n).map_err(|source| CmdConfigError::InvalidField { flag: "workers", source })?,
        ),
    };
    ...
    let input_reports = match args.input {
        ...
        Some(specs) => Some(
            specs
                ...
                .map_err(|source: super::conf_parse::DocumentSpecError| CmdConfigError::InvalidField { flag: "input", source: source.into() })?,
        ),
    };
