//! Rust port of `_internals/cli/cmd.py`'s config-resolution core — the two-pass
//! default→file→env→cmd precedence chain `cmd()` runs before calling `main(config)`. See
//! `agent-memory/rust-native-binary-plan.md`, Fase E, punto 3c.
//!
//! **Deliberately scoped to config resolution only.** `cmd.py` also sets up logging (the stderr
//! handler, and a `.log.csv`-adjacent debug file handler when verbosity is high) and then calls
//! `main(config)`. Neither is ported here: logging setup calls into `core/logging.py`, which is
//! its own unported piece (see `core::tracing_setup`'s doc comment — the CLI's actual log
//! destination handling is explicitly left to Python until that module is ported), and `main()`
//! itself is `cli/main.py` (Fase E, punto 3d), not yet started. Building a Python-object bridge
//! for [`FreeportsConfig`] now, only to replace it once `main.py` is ported and no bridge is
//! needed at all (both sides Rust, called directly), would be throwaway work — so this function
//! stops at producing the resolved [`super::freeports_config::FreeportsConfig`], and the actual
//! `freeports._internals.cli.cmd.cmd` replacement (logging + calling the ported `main`) happens
//! as one combined step once 3d lands.

use super::cmd_config::{self, CliArgs, CmdConfigError};
use super::env_config::{self, EnvConfigError};
use super::file_config::{self, FileConfigError};
...
/// Mirrors `DEFAULT_CONFIG["VERBOSITY"]`, the baseline `-v`/`-q` are applied against.
...
pub fn resolve_partial_config(cli_args: CliArgs) -> Result<PartialConfig, CmdError> {
    let cmd_config = cmd_config::from_args(cli_args, DEFAULT_VERBOSITY)?;
    let env_config = env_config::load()?;
    ...
    let file_config = file_config::load(preliminary.config_file.as_deref())?;
