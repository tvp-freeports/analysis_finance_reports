//! Rust port of `_internals/cli/conf_parse.py`'s `FreeportsEnvConfig` — configuration from
//! `FREEPORTS_*` environment variables. See `agent-memory/rust-native-binary-plan.md`, Fase E,
//! punto 3b-iv.
//!
//! Like `file_config.rs`, `FREEPORTS_URL`/`FREEPORTS_PDF` are replaced by a single
//! `FREEPORTS_INPUT_REPORT` variable parsed with [`super::conf_parse::DocumentSpec`] — see that
//! module's doc comment for why (the originals never actually reached `FreeportsConfig`).
//!
//! Every environment variable is a string, so boolean/numeric/enum values need parsing that a
//! native YAML scalar wouldn't. The Python original defines a `_str_to_bool` helper for this but,
//! reading the whole file, never actually calls it anywhere — `SAVE_PDF` relies on Pydantic's own
//! lax string-to-bool coercion instead, which accepts an equivalent set of aliases. This port
//! parses `SAVE_PDF` directly against that same alias set rather than carrying forward an unused
//! function.

use std::path::PathBuf;

use super::conf_parse::{self, ConfigError, DocumentSpec, OutFlags, OutStructureMode};
