...
//! `PdfLineSelection` (the latter is Rust — `freeports._native.core.PdfLineSelection`, merged in
...
fn pdf_line_selection_class(py: Python<'_>) -> PyResult<Bound<'_, PyAny>> {
    ...
    // `agent-memory/rust-native-binary-plan.md`) and now exported at `freeports._native.core`.
    py.import("freeports._native")?.getattr("core")?.getattr("PdfLineSelection")
}
...
/// empty-input early return). Plain arithmetic — no `Python<'_>`/PyO3 type touched — so it returns
/// a bare message rather than `PyErr`; its one caller (`__call__`, a genuine `#[pymethods]`
/// boundary) converts it via `PyValueError::new_err` at its own `?` site.
fn range_0_to_len_step(len: usize, step: i64) -> Result<Vec<usize>, String> {
    if step == 0 {
...
impl PdfExtractAssetsStandard {
    ...
    fn __call__(&self, py: Python<'_>, dict_root: &Bound<'_, PyAny>) -> PyResult<Vec<Py<PdfBlock>>> {
        ...
        let indices = range_0_to_len_step(tot_assets_items.len(), self.skip_column).map_err(PyValueError::new_err)?;
