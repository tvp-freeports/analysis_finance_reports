...
from freeports_analysis.formats_utils.pdf_filter.pdf_parts.font import FontSizeRange


@standard_pdf_filtering(
    header_set=[
        PdfLineSet(font_size=FontSizeRange(7.7, 8.1), text="Descrizione"),
        PdfLineSet(
            font_size=FontSizeRange(15.45, 15.55), text="TABELLA DEGLI INVESTIMENTI AL"
        ),
    ],
    subfund_set=PdfLineSet(font_size=FontSizeRange(15.45, 15.55), area=(0, 80)),
    body_set=PdfLineSet(font_size=FontSizeRange(7.7, 8.1)),
    currency_set=PdfLineSet(
        font_size=FontSizeRange(7.7, 8.1), text="valori espressi in"
    ),
    deselection_list=[
