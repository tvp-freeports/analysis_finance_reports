...
def tabularize(text_block: Text_Block) -> dict:
    ...
    holdings_match = re.match(
        holdings_regex, "".join(text_block.metadata["holdings"].split())
    )
    net_assets_match = re.match(
        net_assets_regex, "".join(text_block.metadata["% net asset"].split())
    )
    fair_value_match = re.match(
        fair_value_regex, "".join(text_block.metadata["fair value"].split())
    )
    parsed_row = None
    if holdings_match and net_assets_match and fair_value_match:
        ...
        logger.warning(
            "Row not recognized: %s %s", text_block.metadata, text_block.content
        )
        parsed_row = {}
    return parsed_row
