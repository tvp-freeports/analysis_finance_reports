...
from freeports_analysis.formats.utils.pdf_extract.pdf_parts import pdflines_from_pagedict, PdfLineSelection
...
from freeports_analysis.formats.utils.deserialize import DeserializerPageClassifyStandard
...
def PdfExtract_man(page):
    lines = pdflines_from_pagedict(page)
    condition_text = PdfLineSelection(text = "INDEPENDENT AUDITOR OF THE INVESTMENT FUND AND OF THE MANAGEMENT COMPANY", font= "frutiger-lightitalic")
    
    bold_text_up = PdfLineSelection(font = "frutiger-black", font_size = (8.98,8.99)) & PdfLineSelection.area_from_bounds(x0=0,y0=0, x1=1e6, y1=condition_text)
    fund_text = PdfLineSelection(font = "frutiger-lightitalic", font_size = (8.98,8.99)) & PdfLineSelection.area_from_bounds(x0=0,y0=0, x1=1e6, y1=condition_text)
    bold_text_down = PdfLineSelection(font = "frutiger-black", font_size = (8.98,8.99)) & PdfLineSelection.area_from_bounds(x0=0,y0=condition_text, x1=1e6, y1=1e6)
...
def TextFilter(blocks, subfunds):
    ...
    sub = sub.split(')')[:-1]
    for s in sub:
        final.append([e.strip() for e in s.replace('(', '').replace(')', '').split(',')])

    
    sub = final
    final=[]
    return [TextBlock(TipiBlocco.INV,{"funds": s},i) for i,s in zip(inv,sub)] #if any(x in subfunds for x in s)]

def TextFilter_man(blocks,subfunds):

    m = [TextBlock(TipiBlocco.MAN,{},b) for b in blocks if b.type_block ==TipiBlocco.MAN]
    ...
    sub = final
    m.extend([TextBlock(TipiBlocco.INV,{"funds": s},i) for i,s in zip(inv,sub) ])#if any(x in subfunds for x in s)])
    return m
...
header_sets_man = [
    PdfLineSelection(text = "INDEPENDENT AUDITOR OF THE INVESTMENT FUND AND OF THE MANAGEMENT COMPANY", font= "frutiger-lightitalic"),
    PdfLineSelection(text = "ORGANISATION OF THE FUND", font= "frutiger-black")
]
